...
동기식 처리 모델(Synchronous processing model)은 직렬적으로 태스크를 수행한다. 즉, 태스크는 순차적으로 실행되며 어떤 작업이 수행 중이면 다운 작업은 대기하게 된다.

![](https://poiemaweb.com/img/block_nonblock.png)

서버에서 데이터를 가져와서 화면에 표시하는 작업을 수행할 때, 서버에 데이터를 요청하고 데이터가 응답될 때까지 이후 태스크들은 블로킹(작업 중단)된다.
...
<img src="https://poiemaweb.com/img/asynchronous.png" style="zoom:50%;" />
...
<img src="https://poiemaweb.com/img/event-loop.gif" style="zoom:50%;" />
